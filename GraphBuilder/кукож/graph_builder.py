...
def find_skeleton_points_straight(skeleton):
    ...
    for point in points:
        ...
        for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            nx, ny = x + dx, y + dy
            if (
                0 <= nx < skeleton.shape[1]
                and 0 <= ny < skeleton.shape[0]
    ...
    if len(key_points) < 3:
        ...
        additional_points = [
            p[0] for p in neighbor_counts[: min(10, len(neighbor_counts))]
        ]
...
def create_strict_manhattan_path(start, end, green_mask):
    ...
    if is_path_in_green_area(start, intermediate, green_mask) and is_path_in_green_area(
        intermediate, end, green_mask
    ):
        ...
        if is_path_in_green_area(
            start, intermediate, green_mask
        ) and is_path_in_green_area(intermediate, end, green_mask):
            path.append(intermediate)
            path.append(end)
        else:
...
def visualize_results(
    image, G, door_points, corridor_skeleton, green_mask, red_mask, angles_points
):
