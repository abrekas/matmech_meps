...
def process_floor_plan(image_path):
    ...
    # 3. Строим упрощённый скелет коридора
    corridor_skeleton = create_straight_skeleton(green_mask)
...
def create_straight_skeleton(green_mask):
    """Создаем упрощённый прямой скелет коридора"""
    # Находим контур коридора
    contours, _ = cv2.findContours(green_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not contours:
        return np.zeros_like(green_mask)
    
    # Берём самый большой контур (коридор)
    main_contour = max(contours, key=cv2.contourArea)
    
    # Аппроксимируем контур до упрощённой формы
    epsilon = 0.01 * cv2.arcLength(main_contour, True)
    approx_contour = cv2.approxPolyDP(main_contour, epsilon, True)
    
    # Создаём скелет на основе медианной линии
    skeleton = create_median_line_skeleton(green_mask, approx_contour)
    
    return skeleton

def create_median_line_skeleton(mask, contour):
    """Создаем скелет на основе медианной линии контура"""
    # Находим ориентированный bounding rectangle
    rect = cv2.minAreaRect(contour)
    box = cv2.boxPoints(rect)
    box = np.int8(box)
    
    # Сортируем точки для определения направления
    center = rect[0]
    width, height = rect[1]
    angle = rect[2]
    
    # Определяем основное направление коридора
    if width > height:
    ...
    # Создаем упрощённый скелет
    skeleton = np.zeros_like(mask)
    
    if main_axis == 'horizontal':
        # Горизонтальный коридор - создаем горизонтальную линию
        y_center = int(center[1])
        x_start = int(center[0] - long_side/2)
        x_end = int(center[0] + long_side/2)
        cv2.line(skeleton, (x_start, y_center), (x_end, y_center), 255, 2)
    else:
        # Вертикальный коридор - создаем вертикальную линию
        x_center = int(center[0])
        y_start = int(center[1] - long_side/2)
        y_end = int(center[1] + long_side/2)
        cv2.line(skeleton, (x_center, y_start), (x_center, y_end), 255, 2)
    
    # Для извилистых коридоров используем более сложный подход
    if short_side / long_side > 0.3:  # Если коридор имеет значительные изгибы
        skeleton = create_adaptive_skeleton(mask, contour)
    
    return skeleton

def create_adaptive_skeleton(mask, contour):
    """Создаем адаптивный скелет для извилистых коридоров"""
    # Находим основные направления коридора с помощью PCA
    points = contour.reshape(-1, 2).astype(np.float32)
    mean = np.empty(0)
    mean, eigenvectors = cv2.PCACompute(points, mean)
    
    # Основное направление (первая компонента)
    main_direction = eigenvectors[0]
    
    # Проецируем все точки на основное направление
    projected = points - mean
    projections = np.dot(projected, main_direction)
    
    min_proj = np.min(projections)
    max_proj = np.max(projections)
    
    # Создаем точки скелета вдоль основного направления
    skeleton_points = []
    for t in np.linspace(min_proj, max_proj, 50):
        point = mean + t * main_direction
        skeleton_points.append(point[0].astype(int))
    
    # Создаем скелет
    skeleton = np.zeros_like(mask)
    for i in range(len(skeleton_points) - 1):
        pt1 = tuple(skeleton_points[i])
        pt2 = tuple(skeleton_points[i + 1])
        cv2.line(skeleton, pt1, pt2, 255, 2)
